return {
	...
	config = function()
		vim.keymap.set("n", "<leader>dg", ":PetsNewCustom dog brown hi <CR>")
		require("pets").setup({
			row = 3, -- the row (height) to display the pet at (higher row means the pet is lower on the screen), must be 1<=row<=10
			col = 7, -- the column to display the pet at (set to high number to have it stay still on the right side)
